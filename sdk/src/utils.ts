...
export function getDidPrefix(network: Network = 'mainnet'): string {
...
export function extractDidUniqueSuffix(did: string): string {
 ...
 * @returns
 */
export function encodeDidBtc({
  blockHeight,
  txIndex,
  didIndex,
  network = 'mainnet',
}: DidBtcIdentifier): string {
...
function getVerificationMethodsByRelationship(
...
export function buildDidDocument(
  { verificationMethods, controllerKey, metadata }: Did,
  id: string,
): DidDocument {
  ...
  return {
    ...
    controller,
    verificationMethod,
    authentication: getVerificationMethodsByRelationship(
      verificationMethods,
      VerificationRelationshipFlags.AUTHENTICATION,
      id,
    ),
    assertion: getVerificationMethodsByRelationship(
      verificationMethods,
      VerificationRelationshipFlags.ASSERTION,
      id,
    ),
    keyAgreement: getVerificationMethodsByRelationship(
      verificationMethods,
      VerificationRelationshipFlags.KEY_AGREEMENT,
      id,
    ),
    capabilityInvocation: getVerificationMethodsByRelationship(
      verificationMethods,
      VerificationRelationshipFlags.CAPABILITY_INVOCATION,
      id,
    ),
    capabilityDelegation: getVerificationMethodsByRelationship(
      verificationMethods,
 ...
 */
export function isDidBtc(did: string): boolean {
 ...
 */
export function decodeDidBtc(id: string): DidBtcIdentifier {
  if (isDidBtc(id)) {
    const uniqueSuffix = extractDidUniqueSuffix(id);
    const { blockHeight, txIndex, outpoint, network } =
      bip136.decode(uniqueSuffix);
