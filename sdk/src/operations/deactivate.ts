 ...
 * @returns
 */
export function buildDidDeactivationTransaction({
  walletUtxos = [],
  didUtxo,
  didPrivkey,
  satsPerVByte,
  network,
  changeAddress,
}: DidDeactivationParams): BitcoinTransaction {
  ...
  const trKeys = combinedWalletUtxos.map(
    ({ privkey }) => new TaprootKey(privkey),
  );
  ...
  const { fee, inputValue } = calculateTxFeeAndInputValue(
    utxos,
    [{ script: DEACTIVATION_OP_RETURN_OUTPUT, value: 0 }],
    changeOutput,
    satsPerVByte,
  );
