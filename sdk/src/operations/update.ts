 ...
 */
export function buildDidUpdateTransactions({
  didSats = DUST_LIMIT,
  satsPerVByte,
  didUtxo,
  didPrivkey,
  ...
  update,
  did,
}: DidUpdateParams): CommitRevealTransactions {
  assert(
    didSats >= DUST_LIMIT,
 ...
 */
export function buildBatchDidUpdateTransactions({
  didSats = DUST_LIMIT,
  satsPerVByte,
  didUtxo,
  didPrivkey,
  ...
  updates,
  deactivationIndexes,
}: BatchDidUpdateParams): CommitRevealTransactions {
  assert(
    didSats >= DUST_LIMIT,
    'didSats must be greater than 330 sat dust limit',
  );
  ...
  const batchUpdateContent = getBatchUpdateContent(
    updates,
    deactivationIndexes,
  );
...
function validateUpdateContent(update: DidUpdate, did: Did): void {
  assert(!did.isDeactivated, 'Cannot update a deactivated DID');
  if (update.vm !== undefined) {
    update.vm.forEach((vm) => {
      if ('i' in vm) {
        assert(
          vm.i < did.verificationMethods.length,
          `Invalid verification method index ${vm.i}`,
        );
      }
    });
  }
  ...
  if (update.d !== undefined) {
    Object.keys(update.d).forEach((key) => {
      assert(
        did.metadata && key in did.metadata,
        `Metadata key ${key} does not exist in DID document`,
      );
    });
  }
...
function getBatchUpdateContent(
  updates?: BatchDidUpdate[],
  deactivationIndexes?: number[],
) {
  const hasUpdates = updates !== undefined && updates.length > 0;
  const hasDeactivations =
    deactivationIndexes !== undefined && deactivationIndexes.length > 0;
  assert(
    hasUpdates || hasDeactivations,
    'No updates or deactivations provided',
  );
...
function getUpdateContent(update: DidUpdate, did: Did) {
  validateUpdateContent(update, did);
