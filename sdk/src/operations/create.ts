 ...
 */
export function buildDidCreationTransaction({
  multikey,
  walletUtxos,
  satsPerVByte,
  network,
  ...
  didOutput,
  didSats = DUST_LIMIT,
  verificationRelationshipFlags = defaultVerificationRelationshipFlags,
}: DidCreationParams): BitcoinTransaction {
  ...
  assert(
    didSats >= DUST_LIMIT,
    'didSats must be greater than 330 sat dust limit',
  );
  ...
  const changeOutput = getChangeOutput(
    changeAddress,
    walletUtxos[0].privkey,
    network,
  );

  const { fee, inputValue } = calculateTxFeeAndInputValue(
    utxos,
    [
  ...
  const { changeIndex, changeValue } = addChangeIfEconomicallyFeasible(
    tx,
    changeOutput,
    inputValue,
    didSats,
...
function getBatchCreateContent(
  pubkeys: Uint8Array[],
  codec: Uint8Array,
  verificationMethodFlags: number,
) {
 ...
 */
export function buildBatchDidCreationTransactions({
  pubkeys,
  walletUtxos,
  satsPerVByte,
  network,
  ...
  didSats = DUST_LIMIT,
  verificationRelationshipFlags = defaultVerificationRelationshipFlags,
  codec = codecs['ed25519-pub'],
}: BatchDidCreationParams): CommitRevealTransactions {
  assert(walletUtxos && walletUtxos.length > 0, 'walletUtxos must be provided');
  assert(
    didSats >= DUST_LIMIT,
    'didSats must be greater than 330 sat dust limit',
  ...
  assert(
    pubkeys.every((key) => key.length === expectedKeyLength),
    `all pubkeys must be ${expectedKeyLength} bytes`,
  );

  const batchCreateContent = getBatchCreateContent(
    pubkeys,
    codec.prefix,
