...
import { isVerificationMethodAppend, isVerificationMethodDeletion, isVerificationMethodOperation, isVerificationMethodUpdate, } from './types.js';
import { DID_CREATION_OP_RETURN_PREFIX, DIDS_BATCH_CREATION_PAYLOAD_PREFIX, parseRevealPayloadFromWitness, } from './utils.js';
function getDidUpdateFromTransaction(updateTx, didIndex) {
    ...
    if (Array.isArray(payloadJson)) {
        ...
        const didUpdate = payloadJson.find((didUpdate) => {
            assert(typeof didUpdate === 'object' && didUpdate !== null);
            assert('i' in didUpdate && typeof didUpdate.i === 'number' && didUpdate.i >= 0);
        ...
        assert(didIndex === undefined, 'didIndex must be undefined for single DID update');
        assert(isDidUpdate(payloadJson));
        return payloadJson;
    }
}
function applyVerificationMethodOperationToDid(did, vmOp) {
 ...
 */
export function applyUpdateTransactionToDid(did, txHex, didIndex) {
    const tx = Transaction.fromHex(txHex);
    if (tx.outs[0].script.equals(DEACTIVATION_OP_RETURN_OUTPUT)) {
        assert(didIndex === undefined, 'didIndex must be undefined for single DID deactivation');
 ...
 */
export function resolveDidBtc(transactions, didIndex) {
    ...
    if (tx.outs[0].script[0] === 0x6a) {
        ...
        assert(didIndex !== undefined, 'didIndex is required for resolving a DID that was created in a batch');
        ...
        assert(prefix.equals(DIDS_BATCH_CREATION_PAYLOAD_PREFIX), 'Invalid batch creation payload prefix');
        const codec = payload.subarray(4, 6);
        const verificationRelationshipFlags = payload[6];
        assert(verificationRelationshipFlags < 32 && verificationRelationshipFlags > 0, 'Invalid verification relationship flags');
...
function getKeysFromCreateTx(tx) {
    ...
    assert(prefix.equals(DID_CREATION_OP_RETURN_PREFIX), 'Invalid DID creation prefix');
    // 6th byte is the verification relationship flags
    const verificationRelationshipFlags = opReturnOutput[5];
    assert(verificationRelationshipFlags < 32 && verificationRelationshipFlags > 0, 'Invalid verification relationship flags');
...
function isDidUpdate(obj) {
    if (typeof obj !== 'object' || obj === null) {
        return false;
    }
    if ('vm' in obj) {
        if (!Array.isArray(obj.vm) ||
            !obj.vm.every((vm) => isVerificationMethodOperation(vm))) {
            return false;
        }
    }
